...
File a;
	...
	public CompositiveRandoms LoadRandomVariableFromFile(String filename)
	{
		...
		//File filep = new File("D:\\XMLinJava\\src\\"+filename);
		//File filep = new File(filename);
	...
	public static void main(String[] args )
	{
		...
		//List<NodeItem> list = (List<NodeItem>)(test.setXMLFile("testxml.xml").getResult(ex));
		Main Implementationd = new Main();
	        ...
	        //System.out.println("10");
	            Runnable runnable = 
	                        new Runnable() 
	            			{
	                            public void run() 
	                            {	
	                            	List<NodeItem> listn = (List<NodeItem>)(test.setXMLFile("testxml1.xml").getResult(ex));
	                            	list.add(listn.get(0));
	                            	CompositiveMatrix matrixA = Implementationd.LoadMatrixFromFile("testMatrix.txt");
	            					CompositiveRandoms randomVariable = Implementationd.LoadRandomVariableFromFile("testRandom.txt");
	            					//float[] meterInput = { 26.5f, 33.5f ,30.5f ,40,50,60,70,80,90,100};
	            					float[] meterInput = test.inputvalue(test.inputdata(listn));
		...
		if(argc>=2)
		{
			if(args[0].equalsIgnoreCase("testmeterdata"))
			{
				if(argc>=3)
				{
					...
					//float[] meterInput = { 26.5f, 33.5f ,30.5f ,40,50,60,70,80,90,100};
					float[] meterInput = test.inputvalue(test.inputdata(list));
					ArrayList<SingleMeterOutput> meterOutput = Ahe.GetMeterOutput(meterInput, 9, matrixA.getMatrix(), matrixA.getNumberOfRows(), matrixA.getNumberOfColumns(), randomVariable.getRandoms());
